...
variable "proxmox_url" {
  ...
  default     = "https://10.20.0.10:8006/api2/json"
}
...
source "proxmox-iso" "ubuntu-2404" {
  ...
  boot_wait = "20s"
  boot_command = [
    "<esc><wait>",        # raus aus gfxmenu
    "<esc><wait>",        # nochmal sicher
    "<enter><wait>",      # default entry starten
    "/casper/vmlinuz autoinstall ds=nocloud-net;s=http://{{ .HTTPIP }}:{{ .HTTPPort }}/ --- <enter>"
  ]
